...
(function (window, $, Backbone) {
  ...
  var FlipbookView = SingleView.extend({
    ...
    _turnedEvent: function (event, page) {
      AppRouter.getInstance().navigate('page/' + page, {
        trigger: false
      });
...
if (window.innerWidth >= 1100) {
  ...
  const flipdiv = document.querySelector('.catalog-app');
  if (flipdiv) {
...
var isPlaying = false; // Track the play/pause state
$('#playpause').click(function () {
  // Toggle the play/pause icons
  $(this).find('i').toggleClass("fa-pause fa-play");

  if (!isPlaying) {
    // If not playing, start the interval
    startAutoFlip();
    isPlaying = true;
  } else {
    // If playing, stop the interval
    stopAutoFlip();
    isPlaying = false;
  }
});
...
var zoomCenterX = 0.5; // Horizontal center (0.5 means the middle of the element)
var zoomCenterY = 0.5; // Vertical center (0.5 means the middle of the element)
var isZoomedIn = true; // Track the zoom state

$('#zoominout').click(function () {
  if (isZoomedIn) {
    // Zoom out
    zoom -= 0.1;
    $('#flipbook').css('transform-origin', (zoomCenterX * 100) + '% '
      + (zoomCenterY * 100) + '%');
  ...
  // Toggle the icon based on the zoom state
  if (isZoomedIn) {
...
// Function for the second audio (playAudio2)
function playAudio2(audioId, element) {
  var audioElement = document.getElementById(audioId);

  // Check if an audio is currently playing
  if (currentAudio) {
    return; // Do nothing if an audio is still playing
  }

  // Disable the ability to click on another audio link until the audio is fully played
  element.onclick = null;

  // Set up an event listener to re-enable the link when the audio is fully played
  audioElement.addEventListener('ended', function () {
    element.onclick = function () {
      playAudio2(audioId, element);
    };
  });
